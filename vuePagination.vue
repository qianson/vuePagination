      ...
      <div class="items-show fl" @click="handleChooseNumClick"><span class="h50">{{pageNum}}</span>
        <div class="per-page-items">
          ...
          <ul class="items-num" v-if="itemsShow">
            <li v-for='(item,index) in pageSizeSettings' :key='index' @click.stop='chooseNum(item)'>{{item}}</li>
          </ul>
        </div>
      </div>
    </div>
...
export default {
  ...
  props: {
    ...
    pageSizeSettings: { // 配置下拉 选pageSize
      type: Array,
      default () {
        return [5, 10, 15, 20]
      }
  ...
  computed: {
    pageNum () { // 由于父组件传递过来的属性 子组件的钩子里面不能直接使用 用计算属性代替接收
      let a = this.size
      return a
    },
    pageItems () {
  ...
  watch: {
    pageNum () { // 每页数量变化后传递出 pageSize 重新请求数据
    ...
    currentPage () { // 当前页数变化后 传递出当前页码 重新请求数据
      this.pages()
    ...
    totals () { // 数据是异步加载的 组件刚开始totals是默认的是渲染不了的
      this.pages()
    },
    tab () { // 点击切换条件筛选 重置currentPage
      this.currentPage = 1
    }
  }
}
